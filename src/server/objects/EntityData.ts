...
import { AddResult, Team } from "../entities/Team";
...
export class EntityData {
  ...
  addPlayerToTeam(player: Player, newTeam: Team): AddResult {
    if (newTeam.isInQueue(player)) return AddResult.ALREADY_IN_QUEUE;
    if (newTeam.hasPlayer(player)) return AddResult.ALREADY_ON_TEAM;
    
    if (this.isTeamFull(newTeam)) {
      // team is full, add to queue
      newTeam.unsafeAddToQueue(player);
      return AddResult.ADDED_TO_QUEUE;
    }
    ...
    return AddResult.ADDED_TO_TEAM;
  }
